...
const metricNames: Record<Language, Record<string, string>> = {
  ru: Object.fromEntries(INDICATORS.map((metric) => [metric.id, metric.name])),
  ...
  switch (error.code) {
    case "DECISION_COUNT":
      return (validation.decisionCount ?? 0) < 5
        ? t(language, "addMore", { count: 5 - (validation.decisionCount ?? 0) })
        : t(language, "exactlyFive");
    case "DUPLICATE_MEASURE": return t(language, "duplicate");
    case "UNKNOWN_MEASURE": return t(language, "unknown");
