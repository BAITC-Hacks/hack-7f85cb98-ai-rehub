...
type Props = { index: number; decision: Decision; measures: any[]; districts: any[]; disabledMeasureIds: string[]; language: Language; onChange: (index: number, decision: Decision) => void };
...
export default function DecisionSlot({ index, decision, measures, districts, disabledMeasureIds, language, onChange }: Props) {
  ...
  function changeMeasure(measureId: string) {
    ...
    onChange(index, { measureId, districtId: nextMeasure?.type === "district" ? "" : null });
  }
  ...
  return (
    <details className={`plan-decision ${measure ? "is-filled" : ""}`}>
      <summary>
        ...
        <span className="plan-decision-copy"><strong>{measure ? measureName(language, measure.id) : t(language, "emptySlot")}</strong><small>{measure ? `${directionName(language, measure.direction)} · ${district ? districtName(language, district.id) : measure.type === "city" ? t(language, "citywide") : t(language, "chooseDistrict")}` : t(language, "uniqueFive")}</small></span>
      ...
      <div className="plan-decision-fields">
        ...
        {measure?.type === "district" && <label><span>{t(language, "district")}</span><select aria-label={`${t(language, "district")} ${index + 1}`} value={decision.districtId || ""} onChange={(event) => onChange(index, { ...decision, districtId: event.target.value })}><option value="">{t(language, "district")}</option>{districts.map((item) => <option key={item.id} value={item.id}>{districtName(language, item.id)}</option>)}</select></label>}
        {measure?.type === "city" && <p>{t(language, "cityMeasure")}</p>}
        {measure && <p>{t(language, "lag")}: {measure.lag} {t(language, "quarter")}</p>}
      </div>
    </details>
  );
}
